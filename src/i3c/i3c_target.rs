...
use proposed_traits::i3c_target::{I3CCoreTarget, DynamicAddressable, IBICapable};
...
impl<HW: HardwareInterface, L: crate::common::Logger> I2CCoreTarget for I3cController<HW, L> {
    ...
    fn init(&mut self, own_addr: u8) -> Result<(), Self::Error> {
        if let Some(t) = self.config.target_config.as_mut() {
            if t.addr.is_none() { t.addr = Some(own_addr); }
        } else {
            self.config.target_config = Some(I3cTargetConfig::new(0, Some(own_addr), /*mdb*/ 0xae));
        }
        Ok(())
    }
...
impl<HW: HardwareInterface, L: crate::common::Logger> IBICapable for I3cController<HW, L> {
    ...
    fn get_ibi_payload(&mut self, buffer: &mut [u8]) -> Result<usize, Self::Error> {
        let (da, mdb) = match self.config.target_config.as_ref() {
            Some(t) => (match t.addr { Some(da) => da, None => return Ok(0) }, t.mdb),
            None => return Ok(0),
        };
        ...
        let rc = self.hw.target_pending_read_notify(&mut self.config, buffer, &mut ibi);

        match rc {
            0 => return Ok(buffer.len()+ payload.len()),
            _ => Ok(0),
        }
    }
...
fn crc8_ccitt(mut crc: u8, data: &[u8]) -> u8 {
    for &b in data {
        let mut x = crc ^ b;
        for _ in 0..8 { x = if (x & 0x80) != 0 { (x << 1) ^ 0x07 } else { x << 1 }; }
        crc = x;
    }
    crc
}
