...
pub fn ccc_events_all_set<H>(hw: &mut H, config: &mut I3cConfig, enable: bool, events: u8) -> i32
where H: HardwareInterface
{
    let id = if enable { i3c_ccc_enec(true) } else { i3c_ccc_disec(true) };
    hw.do_ccc(config, &mut CccPayload {
        ccc: Some(Ccc { id, data: Some(&mut [events]), num_xfer: 0 }),
        targets: None,
    })
}

pub fn ccc_events_set<H>(hw: &mut H, config: &mut I3cConfig, da: u8, enable: bool, events: u8) -> i32
where H: HardwareInterface
{
    if da == 0 {
        return -22
    ...
    let ret = hw.do_ccc(config, &mut payload);

    ret
}

pub fn ccc_rstact_all<H>(hw: &mut H, config: &mut I3cConfig, action: CccRstActDefByte) -> i32
where H: HardwareInterface
{
...
pub fn ccc_getbcr<H>(hw: &mut H, config: &mut I3cConfig, dyn_addr: u8) -> Result<u8, i32>
where H: HardwareInterface
{
    if dyn_addr == 0 {
...
pub fn ccc_getpid<H>(hw: &mut H, config: &mut I3cConfig, dyn_addr: u8) -> Result<u64, i32>
where H: HardwareInterface
{
...
pub fn ccc_getstatus<H>( hw: &mut H, config: &mut I3cConfig, da: u8, fmt: GetStatusFormat,) -> Result<GetStatusResp, i32>
where H: HardwareInterface
{
...
pub fn ccc_getstatus_fmt1<H>(hw: &mut H, config: &mut I3cConfig, da: u8) -> Result<u16, i32>
where H: HardwareInterface {
...
pub fn ccc_rstdaa_all<H>(hw: &mut H, config: &mut I3cConfig) -> i32
where H: HardwareInterface
{
    hw.do_ccc(config, &mut CccPayload {
